    ...
    if eventName == "Cinematics" then
        ...
        if tonumber(barProperties[1]) > 0 then
            ...
            for i = 0, 7 do
                ...
                if moveNotes then
                    noteTweenY("noteY" .. i, i, downscroll and (screenHeight - tonumber(barProperties[1])) - (getPropertyFromGroup("strumLineNotes", i, "height") + 20) or tonumber(barProperties[1]) + initialStrumY, tonumber(barProperties[2]), "quadOut")
                end
            end
        else
            fadeHUD = true
